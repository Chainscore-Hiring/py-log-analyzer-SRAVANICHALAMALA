...
class Worker:
    ...
    async def report_health(self) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.coordinator_url}/health", json={"worker_id": self.worker_id}) as response:
                await response.text()
    ...
    async def run_server(self, port: int):
        ...
        await site.start()
        print(f"Worker {self.worker_id} is running on port {port}...")
        while True:
...
if __name__ == "__main__":
    ...
    asyncio.run(worker.run_server(args.port))
